 ...
 * @param {object} props 
 */
const LineChartDailyRec = observer((props) => {
      ...
      <VictoryChart width={700} height={300}
        ...
        containerComponent={
          <VictoryVoronoiContainer voronoiDimension="x"
            labels={({ datum }) => {
              return datum.y !== 0 ? `${datum.labelTooltip} ${ props.formatThous ? Math.round(datum.y) : hlp[formatFnName]( datum.y )+'m' }` : null
            }}
            style={{
              fontSize: 8, 
              overflow:'visible', 
...
export default LineChartDailyRec
