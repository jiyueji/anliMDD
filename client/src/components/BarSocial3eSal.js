...
const BarSocial3eSal = observer((props) => {
  if (!props.data) {
    return false
  }
...
//  const needFormatThous = !!props.formatThous
  ...
  const COLOR_SCALE = props.customColors || [ "#31BEF9", "#59B961" ]
  const legendsText = props.legendsText || ['3E Sales', '3E Sales of Total Sales']
  const BAR_WIDTH = 20
  ...
  return   <div>
      ...
      <div className="main-block">
              ...
              <label className="st-card-tl-sm float-right">Percentage of Total Sales</label>

              <VictoryChart width={1300} height={300} 
                theme={hlp.smallChartTheme}
                domainPadding={{x: [20, 0], y: 50}} 
                padding={{left: 80, top: 5, right: 100, bottom: 60}}

                domain={{ y: [0, 1] }}

                containerComponent={
                  <VictoryVoronoiContainer voronoiDimension="x"
                    labels={({ datum }) => datum.y > 100 ? hlp.toShortMil( datum.y )+'m' :Math.round( datum.y )+'%' }
                    // labels={({ datum }) => {
                ...
                <VictoryLegend x={365} y={270}
                  ...
                  data={[
                    { name: `${legendsText[0]} ${maxYearStr}`, symbol: {type: 'square'} },
                    { name: `${legendsText[1]} ${maxYearStr}`, symbol: {type: 'minus'} }
                  ]}
                />
